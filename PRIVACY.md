...
classes, and permitted attribute values may contain secrets, personal data, or
other sensitive application data. Do not enable Inspect on sensitive pages
...
headers, form values, DOM text, or general page content. Browser source facts
do not leave the linked local VS Code process. Clipboard contents are read only
after the user clicks Paste in the DevTools panel, and are used only to fill the
link-code field.
...
| `http://127.0.0.1/*` and `http://localhost/*` | Permits the extension runtime to reach the linked loopback bridge; product traffic uses loopback WebSocket only. |
